...
**Protocol-driven architecture:** All layers use `typing.Protocol` for interfaces (not ABC). Data providers, stock pool, and notification each have a dedicated `protocol.py`; factor engines, the LLM judge engine, and backtest `TargetProvider` define their Protocol inline next to the consumers (`factor/technical.py`, `strategy/llm/client.py`, `backtest/target_provider.py`). New implementations must satisfy the Protocol, not inherit from a base class.
...
**Components dependency injection:** All runtime dependencies are assembled into a `Components` dataclass in `src/pangu/scheduler.py` (built via `pangu.main.build_components()`) and injected into `TradingScheduler` and tasks. Tests use fake implementations (`tests/fakes.py`). No global singletons for services.
...
|---------|---------|---------|
| `pangu run init` | One-shot cold-start (backfill → compute-factors → train → init portfolio_state). Idempotent; `--force` re-runs all steps | varies |
