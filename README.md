...
| T2 新闻采集+LLM | 07:00–20:00 | 每小时 | 国内+国际新闻/公告 → 批量 LLM 分析 → 事件信号 |
| T3 国内行情同步 | 15:30 | 每交易日 1 次 | 自选股+CSI300 日 K 线 + 全量基本面 |
| T4 信号生成 | 08:15 (T1 后) | 每交易日 1 次 | 因子排名 + 事件融合 + 事后验证 → 推送"今日操作建议" |
...
trading-agent run news               # T2: 新闻采集+LLM分析
