...
class LLMJudgeEngineImpl:
    ...
    Uses bull/bear/judge three-role debate (inspired by PanGus).
    """
    ...
    async def judge_rebalance(
        ...
        *,
        ...
        back to the classic ML-rank-based TopkDropout.
        """
        # Normalize: filter factor_details to whitelist, defaults for safety
    ...
    @staticmethod
    def _parse_picks(
        raw: Any, *, allowed: set[str], n_drop: int,
    ) -> list[Pick]:
        ...
        for item in raw:
            ...
            if not reason:
                # Reason is required by the schema; skip empty reasons.
                continue
