...
class TestCandidatePools:
    ...
    def test_invalid_sizes_raise(self):
        with pytest.raises(ValueError):
            MLScoringStrategy(
                _make_scorer_mock({}),
                top_n=25, buy_candidate_size=2, sell_candidate_size=5, n_drop=3,
            )
        with pytest.raises(ValueError):
            MLScoringStrategy(
                _make_scorer_mock({}),
                top_n=25, buy_candidate_size=10, sell_candidate_size=2, n_drop=3,
            )
...
class TestFallback:
    def _strat(self):
        return MLScoringStrategy(
            _make_scorer_mock({}),
            top_n=25, buy_candidate_size=5, sell_candidate_size=5, n_drop=3,
        )
    ...
    def test_fallback_sells_skips_excluded(self):
        strat = self._strat()
        pool_df = pd.DataFrame({
            "symbol": ["A", "B", "C"], "score": [0.9, 0.8, 0.7], "rank": [1, 2, 3],
        })
    ...
    def test_fallback_sells_missing_rank_treated_as_worst(self):
        ...
        strat = self._strat()
        pool_df = pd.DataFrame({
            "symbol": ["A", "B"], "score": [0.9, 0.8], "rank": [1, 2],
        })
...
class TestColdStart:
    def test_cold_start_picks_top_n_by_rank(self):
        strat = MLScoringStrategy(
            _make_scorer_mock({}),
            top_n=3, buy_candidate_size=5, sell_candidate_size=5, n_drop=3,
        )
        pool_df = pd.DataFrame({
            "symbol": ["A", "B", "C", "D", "E"],
            "score": [0.9, 0.8, 0.7, 0.6, 0.5],
